...
def _draw_band(ax, intersection_positions: list, programs: dict, speeds: dict, offsets: list, time_max, light_cycles, direction="inbound", color="green"):
    """
...
def _draw_light(ax, position, cycle, time_max, program):
...
def start_multi_algorithm(
    ...
    light_cycles = {
        number:sum([value for color_key, value in dict_bounds["inbound"] if color_key != "yellow"]) for number, dict_bounds in programs.items()
    }
    ...
    _draw_band(ax, intersection_positions, programs, speeds, offsets, time_max, light_cycles, direction="inbound", color="green")
    _draw_band(ax, intersection_positions, programs, speeds, offsets, time_max, light_cycles, direction="outbound", color="blue")

    colors = ["purple", "cyan", "orange", "magenta", "lime"]

    # Graficar múltiples DataFrames Inbound y Outbound
    if df_in_dict is not None and df_out_dict is not None:
        for idx in range(1, len(df_in_dict) + 1):
            color = colors[(idx-1) % len(colors)]

            # Graficar Inbound
            if idx in df_in_dict and df_in_dict[idx] is not None:
                ax.plot(df_in_dict[idx]["Time_Seconds"], df_in_dict[idx]["Distance"],
                        marker='o', linestyle='-', color=color, label=f"Recorrida Ida {idx}")
                
            # Graficar Outbound
            if idx in df_out_dict and df_out_dict[idx] is not None:
                ax.plot(df_out_dict[idx]["Time_Seconds"], df_out_dict[idx]["Distance"],
                        marker='o', linestyle='-', color=color, label=f"Recorrida Vuelta {idx}")
